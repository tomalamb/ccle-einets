    ...
    def em_update(self, _triggered=False):
        ...
        with torch.no_grad():
            ...
            if self.online_em_stepsize is None:
                self.params.data.copy_(n)
            else:
                s = self.online_em_stepsize
                p = torch.clamp(n, 1e-16)
                p = p / p.sum(self.normalization_dims, keepdim=True)
                self.params.data.copy_((1. - s) * self.params.data + s * p)

            self.params.data.copy_(torch.clamp(self.params.data, 1e-16))

            if self.params_mask is not None:
                self.params.data.copy_(self.params.data * self.params_mask)

            self.params.data.copy_(self.params.data / self.params.data.sum(self.normalization_dims, keepdim=True))
            self.params.grad = None
